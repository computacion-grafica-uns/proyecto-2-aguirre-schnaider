Shader "ToonGeneradoCuero"
{
    ...
    SubShader
    {
        ...
        Pass
        {
           ...
           // Procedural leather color using _BaseColor and a lighter grain
            float3 ProceduralLeather(float2 uv)
            {
                float grain = grainPattern(uv);
                // Make the grain color a lighter version of _BaseColor
                float3 grainColor = lerp(_BaseColor.rgb, float3(1,1,1), 0.25); // 25% towards white
                return lerp(_BaseColor.rgb, grainColor, grain * _LeatherGrainStrength);
            }
            ...
            {
                float3 texColor = Posterize(ProceduralLeather(i.uv), _ShadeLevels*2);
